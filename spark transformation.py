...
job = Job(glueContext)
s3_path="s3://spotify-etl-data-mudasir/raw_data/to_be_processed_data/"
...
df = df.withColumn("items", explode("items")).select(
        ...
        col("items.track.album.external_urls.spotify").alias("album_url")).show(5)
...
def write_to_s3(df, path_suffix, format_type="csv"):
    ...
    glueContext.write_dynamic_frame.from_options(
        frame=dynamic_frame,
        connection_type="s3",
        connection_options={"path": f"s3://spotify-etl-data-mudasir/transformed_data/{path_suffix}/"},
        format=format_type,
...
write_to_s3(artists_df, "artist_data/artists_transformed_{}".format(datetime.now().strftime("%Y-%m-%d")),"csv")
